$(function () {
	...
	function getlist(e) {
		$.ajax({
			url : "/newshop/shopadmin/getshoplist",
			type : "get",
			dataType : "json",
	...
	function goShop(status, id) {
		if (status != 0 && status != -1) {
			return '<a href="/newshop/superadmin/shopmanage?shopId='+ id +'">进入</a>';
		} else {
			return '';
		}
	}
	...
	$('#log-out').click(function () {
		$.ajax({
			...
			success : function(data) {
				if (data.success) {
					window.location.href = '/newshop/shopadmin/ownerlogin';
				}
			},
